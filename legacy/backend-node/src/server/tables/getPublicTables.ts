...
export const getPublicTables = async (utc : Promise<string>, user_id : string) : Promise<any> => {
    const sql = `
        SELECT pt.*
            FROM (
                SELECT t.id, t.name, t.description, CONVERT_TZ(t.startdate, t.timezone, ?) as startdate, GROUP_CONCAT(u.name) as masters
                    FROM Tables t 
