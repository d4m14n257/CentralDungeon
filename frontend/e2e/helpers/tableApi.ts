...
export interface RunnableTableBody {
  ...
  systemIds: string[]
  platformIds: string[]
  schedule: { weekday: string; hourtime: string }[]
  [key: string]: unknown
}
 ...
 *
 * A table cannot be created without saying what is played, where, and when (#226), so a spec that
 * only cares about what happens *after* the table exists still has to carry all three. This is that
 * minimum, in one place, so the next required field is one edit instead of one per spec.
 *
...
export function runnableTableBody(name: string, extra: Record<string, unknown> = {}): RunnableTableBody {
  return {
    ...
    schedule: [{ weekday: 'Friday', hourtime: '20:00' }],
    ...extra,
  }
}
