...
| `docs/decisiones.md` | Qué se decidió y por qué. Se consulta cuando algo parece arbitrario, y se actualiza cuando una decisión cambia. 165 decisiones cerradas y el rastro de los 31 pendientes que se abrieron y resolvieron (M1–M31). |
...
5. **Cuatro roles acumulables y sin jerarquía**: `Player`, `Master`, `Admin`, `Owner`. Única excepción, `Owner` puede todo lo de `Admin`. **No se registra un `RoleHierarchy`**: cada endpoint enumera sus roles (`hasAnyRole('ADMIN','OWNER')`). Y ojo con los dos `Owner`: el rol de plataforma y `masters.master_type = 'Primary'` no son lo mismo y no se llaman igual (`docs/decisiones.md` #67, #71, #89).
